...
Select id ,name,
Max(price) over(order by price ROWS BETWEEN CURRENT ROW AND 1 FOLLOWING)
as Max
from products;

Select invoice_id, 
avg(i.total_amount) 
over (order by i.total_amount rows 2 preceding)
as Average
from invoices i;


----------------------------------
Select column_value from the (select phone_numbers from users  where id=2 );

UPDATE TABLE (select phone_numbers from users  where id=2) c
  SET VALUE(c) = 44293198
  WHERE  VALUE(c) = 44292198;

Delete from table(select phone_numbers from users  where id=2)c where value(c)=44293198;


Select column_value from the (Select c.product_items from Cart c ,users u where c.client_id=u.id and u.name.firstname='Endrit');


---------Recursive Query-----------


SELECT discount_id, id, level
  FROM products
CONNECT BY PRIOR id = discount_id
START WITH id = 1
